...
resource "aws_lambda_function" "upload" {
  ...
  environment {
    variables = {
      FILE_UPLOAD_BUCKET = data.aws_s3_bucket.intake.id
    }
  }
  package_type = "Zip"
  runtime = "nodejs14.x"
  ...
  policy = jsonencode(
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": [
                "arn:aws:s3:::${data.aws_s3_bucket.intake.id}/*",
            ]
        },
        {
            "Sid": "AllowS3Bucket",
            ...
            "Resource": "arn:aws:s3:::${data.aws_s3_bucket.intake.id}"
        },
        {
            "Sid": "AllowKMS",
            "Effect": "Allow",
...
resource "aws_api_gateway_authorizer" "upload" {
  ...
  #provider_arns = ["arn:aws:cognito-idp:${var.aws_region}:${var.aws_account_number}:userpool/${data.aws_cognito_user_pools.user_pool.arns}"]
